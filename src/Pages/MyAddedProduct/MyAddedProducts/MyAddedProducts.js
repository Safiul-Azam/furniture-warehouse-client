...
const MyItem = () => {
  ...
  useEffect(() => {
    const getMyItem = async () => {
      ...
      const url = `https://furniture-9cwv.onrender.com/myItems?email=${email}`;
      console.log(url);
      try {
  ...
  return (
    <div>
      <div className="d-flex align-items-center w-25 mx-auto">
        ...
        <div>
          <h3 className="text-center my-4 mx-3">
            My <span className="title-color">Item</span>
          </h3>
        </div>
        <div style={{ height: "2px" }} className="line-color w-25"></div>
      </div>
      {myItem.map((furniture) => (
        <MyAddedProduct
          key={furniture._id}
          furniture={furniture}
